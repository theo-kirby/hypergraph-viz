...
// groups: [{title, rows: [{key, label, type: "range"|"check"|"segment",
//                          min?, max?, step?, options?: [{value, label}]}]}]
export function createPanel({ mount, groups, settings, onChange }) {
  injectStyle();
  ...
  head.innerHTML = '<span>⚙ Tune</span><span class="tp-caret">▾</span>';
  ...
  groups.forEach((group) => {
    ...
    group.rows.forEach((row) => {
      ...
      if (row.type === "range") {
        ...
        const show = () => { val.textContent = String(settings[row.key]); };
        input.addEventListener("input", () => {
          settings[row.key] = parseFloat(input.value);
          show();
          fire(row.key, false);
        });
        refreshers.push(() => { input.value = settings[row.key]; show(); });
        line.append(input, val);
      } else if (row.type === "check") {
        const input = document.createElement("input");
        input.type = "checkbox";
        input.addEventListener("change", () => {
          settings[row.key] = input.checked;
          fire(row.key, true);
        });
        refreshers.push(() => { input.checked = !!settings[row.key]; });
        ...
        const paint = () => buttons.forEach(({ b, opt }) => b.classList.toggle("on", settings[row.key] === opt.value));
