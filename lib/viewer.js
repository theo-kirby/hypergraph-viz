...
export function createViewer(cfg) {
  ...
  function placeNode(el, c, n) {
    el.style.left = c.cx - n.w / 2 + "px";
    el.style.top = c.cy - n.h / 2 + "px";
  ...
  function sizeSvg(bbox) {
    svg.setAttribute("width", Math.ceil(bbox.minX + bbox.w + 200));
    svg.setAttribute("height", Math.ceil(bbox.minY + bbox.h + 200));
  }
  ...
  const onCanvas = (e) => !e.target.closest(".node") && !e.target.closest(".tunepanel");
  ...
  stage.addEventListener("pointerdown", (e) => {
    ...
    const move = (ev) => applyView({ k: v0.k, x: v0.x + ev.clientX - sx, y: v0.y + ev.clientY - sy });
    const up = () => {
