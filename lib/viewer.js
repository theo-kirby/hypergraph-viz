...
export function createViewer(cfg) {
  ...
  let minZoom = 0.15; // manual zoom-out limit; follows the fit on big graphs
  ...
  function computeFit(bbox) {
    ...
    // No zoom-out floor: an auto-fitted big project can be arbitrarily wide,
    // and landing fitted matters more than a minimum node size. Manual zoom
    // may go a step past the fit, so a fitted view can still be pulled back.
    const k = Math.min(1.4, (sw - 2 * stagePad) / bbox.w, (sh - 2 * stagePad) / bbox.h);
    minZoom = Math.min(0.15, k * 0.5);
  ...
  function refit(autoOnly = false) {
    ...
    if (!autoOnly) userAdjusted = false; // an explicit refit hands the view back
    if (!lastView) return;
    const nodeById = {};
  ...
  stage.addEventListener("wheel", (e) => {
    ...
    const k = Math.max(minZoom, Math.min(5, curView.k * factor));
    const r = k / curView.k;
  ...
  // The live pan/zoom, for consumers that need the on-screen window in plane
  // coordinates (the screen-crop PNG export).
  const view = () => ({ ...curView });

  return { transition, refit, view };
}
