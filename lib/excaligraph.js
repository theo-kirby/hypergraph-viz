...
// ---- radial layout: equal-split circle, fans centred on the parent spoke ---
// The n rank-1 nodes split the full circle equally; each owns a 2π/n wedge.
// Deeper nodes fan out centred on the parent's spoke (the ray from the root
// through the parent), each fan clamped to its parent's wedge. A single child
// continues exactly straight, so a chain collapses to a line.
export function radialLayout(nodes, edges, options = {}) {
  ...
  const forest = buildForest(nodes, edges);
  const byId = new Map(nodes.map((n) => [n.id, n]));
  const extent = (id) => { const n = byId.get(id); return Math.max(n.width, n.height); };

  const centres = new Map();
  // place(node) at polar (depth * ranksep, theta); it owns `wedge` radians.
  const place = (id, depth, theta, wedge) => {
    ...
    const delta = Math.min((Math.max(...kids.map(extent)) + nodesep) / rChild, wedge / m);
    kids.forEach((kid, i) => {
      place(kid, depth + 1, theta + (i - (m - 1) / 2) * delta, wedge / m);
    });
  };

  // One root sits at the origin; several roots become ring 1 around a
  // virtual origin. The root stays pinned at (0, 0) — no re-centring — so
  // folding a branch never shifts the rest of the graph: coordinates may go
  // negative, which every consumer (fit, routing, export) handles.
  ...
  if (centreId) centres.set(centreId, { x: 0, y: 0 });
  firstRing.forEach((id, i) => {
    place(id, 1, start + (i * TAU) / firstRing.length, TAU / firstRing.length);
  });

  centres.forEach((p, id) => out.set(id, { cx: p.x, cy: p.y }));
  return out;
}
...
// The points for one bow edge leaving along `normal` (the spoke direction).
//   start: on the source outline at (spoke angle + fan-index * originSpread),
//          so sibling arrows leave clustered around the spoke exit point;
//   mid:   bowed sideways so the curve departs tangent to the spoke
//          (circular-arc sagitta (L/2)*tan(alpha/2), scaled by bowScale);
//   end:   pulled back from the target centre toward the mid point, so the
//          final segment (and the arrowhead) aims at the target's centre.
// Returns null when the geometry degenerates (overlapping boxes).
function bowPoints(source, target, normal, fanIndex, opts) {
  const gap = opts.gap ?? GAP;
  const originSpread = ((opts.originSpread ?? 0) * Math.PI) / 180;
  const bowScale = opts.bowScale ?? 1;
  const sc = centerOf(source), tc = centerOf(target);
  const phi = Math.atan2(normal[1], normal[0]) + fanIndex * originSpread;
  const start = applyGap(
    outlinePoint(source.shape || "rectangle", source, [sc[0] + Math.cos(phi), sc[1] + Math.sin(phi)]),
    sc, gap,
  );

  // provisional end: on the target outline, toward the start point
  const e0 = applyGap(outlinePoint(target.shape || "rectangle", target, start), tc, gap);
  const dx = e0[0] - start[0], dy = e0[1] - start[1];
  const len = Math.hypot(dx, dy);
  if (len < EPS) return null;
  const alpha = Math.atan2(normal[0] * dy - normal[1] * dx, normal[0] * dx + normal[1] * dy);
  const bow = (-len / 2) * Math.tan(alpha / 2) * bowScale;
  if (!isFinite(bow)) return null;
  if (Math.abs(bow) < 0.5) return [start, e0];

  const mid = [(start[0] + e0[0]) / 2 - (dy / len) * bow, (start[1] + e0[1]) / 2 + (dx / len) * bow];
  const end = applyGap(outlinePoint(target.shape || "rectangle", target, mid), tc, gap);
...
// opts: {style: "branch"|"bow", gap, radius, lean, spread, originSpread,
// bowScale}. "branch" runs out then turns; "bow" (for radial views) leaves
// tangent to the spoke and curves to the target — it only applies to edges
// with an explicit normal; the rest go straight, centre to centre.
// An edge may carry its own overrides in e.routeOpts (merged over opts) —
// a combined view routes each side with that side's own tuning.
// Returns Map edgeId -> absolute points (2 straight, 3 bow, 4 branch).
export function routeEdges(edges, boxOf, normalFor, opts = {}) {
  const style = opts.style ?? "branch";
  const spread = opts.spread ?? BRANCH_SPREAD;
  const gap = opts.gap ?? GAP;
  const normals = new Map(), explicit = new Set();
  edges.forEach((e) => {
  ...
  const starts = new Map(), fanIndex = new Map();
  groups.forEach((group) => {
    ...
    ordered.forEach((e, i) => fanIndex.set(e, i - (ordered.length - 1) / 2));
  ...
  edges.forEach((e) => {
    const s = boxOf(e.source), t = boxOf(e.target);
    if (!s || !t) return;
    const eo = e.routeOpts ? { ...opts, ...e.routeOpts } : opts;
    if ((eo.style ?? style) === "bow" && explicit.has(e)) {
      const bow = bowPoints(s, t, normals.get(e), fanIndex.get(e) || 0, eo);
      if (bow) { routes.set(e.id, bow); return; }
    } else {
      const branch = branchPointsAlong(s, t, normals.get(e), starts.get(e) || 0, eo);
      if (branch) { routes.set(e.id, branch); return; }
    }
    const sc = centerOf(s), tc = centerOf(t);
    routes.set(e.id, [
      attachmentPoint(s.shape || "rectangle", s, tc, eo.gap ?? gap),
      attachmentPoint(t.shape || "rectangle", t, sc, eo.gap ?? gap),
    ]);
  });
  return routes;
}
